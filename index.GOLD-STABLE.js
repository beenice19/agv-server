...
const SUPER_ADMIN_IDS = ["byron-admin"];
const SUPER_ADMIN_DISPLAY_NAMES = ["Byron", "byron", "Byron Avant", "byron avant"];
...
function isSuperAdmin(actor) {
  const actorUserId = normalizeLower(actor?.userId);
  const actorDisplayName = normalizeLower(actor?.displayName);

  return (
    SUPER_ADMIN_IDS.map(normalizeLower).includes(actorUserId) ||
    SUPER_ADMIN_DISPLAY_NAMES.map(normalizeLower).includes(actorDisplayName)
  );
}
...
app.delete("/api/rooms/:roomId", (req, res) => {
  ...
  const permissions = buildPermissions(room, actor);
  if (!permissions.isAssignedHost && !permissions.isSuperAdmin) {
