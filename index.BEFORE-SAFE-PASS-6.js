...
const DEFAULT_ROOM_STATE = {
  "main-hall": {
    messages: [
      { id: 1, sender: "System", text: "Welcome to Avant Global Vision.", time: timeNow() },
...
function saveData() {
  fs.writeFileSync(DATA_FILE, JSON.stringify({ rooms, roomState }, null, 2), "utf8");
}
...
function loadData() {
  ...
  try {
    ...
    roomState = parsed.roomState && typeof parsed.roomState === "object"
      ? parsed.roomState
...
function getRole(room, authUser) {
  ...
  if (room.assignedHost === authUser.displayName) return "host";
  if (Array.isArray(room.moderators) && room.moderators.includes(authUser.displayName)) {
    return "moderator";
  }
  return "viewer";
}
...
function emitRooms() {
  io.emit("rooms:update", { rooms: rooms.map(normalizeRoom) });
}
...
app.post("/api/auth/register", requireAuth, requireSuperadmin, (req, res) => {
  ...
  const password = String(req.body?.password || "");
  const globalRole = cleanName(req.body?.globalRole) === "superadmin" ? "superadmin" : "user";
  ...
  if (users.some((user) => user.displayName === displayName)) {
    return res.status(409).json({ ok: false, error: "Display name already exists" });
  }
...
app.post("/api/admin/users/:username/deactivate", requireAuth, requireSuperadmin, (req, res) => {
  ...
  if (user.username === DEFAULT_ADMIN_USERNAME) {
    return res.status(400).json({ ok: false, error: "Cannot deactivate default admin" });
  }
...
app.post("/api/admin/users/:username/reactivate", requireAuth, requireSuperadmin, (req, res) => {
  const username = cleanName(req.params.username).toLowerCase();
  const user = users.find((entry) => entry.username.toLowerCase() === username);
...
app.get("/api/rooms", requireAuth, (req, res) => {
  return res.json({ ok: true, rooms: rooms.map(normalizeRoom) });
});
...
app.post("/api/rooms", requireAuth, (req, res) => {
  ...
  return res.json({ ok: true, room: normalizeRoom(room) });
});
...
app.get("/api/rooms/:roomId/state", requireAuth, (req, res) => {
  ...
  if (!canEnterRoom(room, req.authUser)) {
    return res.status(403).json({ ok: false, error: "Room is locked" });
  }

  return res.json({ ok: true, state: ensureRoomState(room.id) });
});
...
app.post("/api/rooms/:roomId/assign-host", requireAuth, (req, res) => {
  ...
  if (req.authUser.globalRole !== "superadmin") {
    return res.status(403).json({ ok: false, error: "Only Admin can assign a room host" });
  }

  const nextHostDisplayName = cleanName(req.body?.displayName || req.body?.user);
  if (!nextHostDisplayName) {
    return res.status(400).json({ ok: false, error: "Host display name is required" });
  }
  ...
  room.moderators = uniqueNames(room.moderators).filter((name) => name !== targetUser.displayName);
...
app.post("/api/rooms/:roomId/add-moderator", requireAuth, (req, res) => {
  ...
  const nextModeratorDisplayName = cleanName(req.body?.displayName || req.body?.user);
  if (!nextModeratorDisplayName) {
    return res.status(400).json({ ok: false, error: "Moderator display name is required" });
  }
  ...
  if (targetUser.displayName !== room.assignedHost) {
    room.moderators = uniqueNames([...(room.moderators || []), targetUser.displayName]);
  }
...
app.post("/api/rooms/:roomId/remove-moderator", requireAuth, (req, res) => {
  ...
  const moderatorDisplayName = cleanName(req.body?.displayName || req.body?.user);
  if (!moderatorDisplayName) {
    return res.status(400).json({ ok: false, error: "Moderator display name is required" });
  }

  room.moderators = uniqueNames(room.moderators).filter((name) => name !== moderatorDisplayName);
...
app.post("/api/rooms/:roomId/bulletins/add", requireAuth, (req, res) => {
  ...
  const text = cleanName(req.body?.text);
  if (!text) {
    return res.status(400).json({ ok: false, error: "Bulletin text is required" });
  }
...
app.post("/api/rooms/:roomId/bulletins/import", requireAuth, (req, res) => {
  ...
  if (lines.length === 0) {
    return res.status(400).json({ ok: false, error: "No bulletin lines provided" });
  }
...
app.post("/api/rooms/:roomId/presence/heartbeat", requireAuth, (req, res) => {
  const room = findRoom(req.params.roomId);
...
server.listen(PORT, () => {
  ...
  if (!usersFileExists) {
    ...
    console.log("DEFAULT ADMIN PASSWORD is loaded from AGV_ADMIN_PASSWORD or the fallback in index.js.");
