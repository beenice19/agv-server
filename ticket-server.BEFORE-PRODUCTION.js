...
app.get("/api/tickets/health", (req, res) => {
  res.json({ ok: true, service: "AGV Ticket Server", port: PORT });
});
...
app.post("/api/tickets/verify", (req, res) => {
  ...
  if (!code) {
    return res.status(400).json({ ok: false, message: "Ticket code required." });
  }

  const data = readTickets();
  const ticket = data.tickets.find((t) => String(t.code).toUpperCase() === code);

  if (!ticket) {
    return res.status(404).json({ ok: false, message: "Invalid ticket code." });
  }
