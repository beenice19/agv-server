...
// PASS_BCAST6_CLOUDFLARE_RECORDING_MODE_CONTROL
// SERVER — Cloudflare recording mode control.
// Lets AGV set Cloudflare Stream live input recording mode to off or automatic.
// Does not expose API tokens or stream keys.

function agvCloudflareRecordingConfig() {
  const accountId =
    process.env.AGV_CLOUDFLARE_ACCOUNT_ID ||
    process.env.CLOUDFLARE_ACCOUNT_ID ||
    "";
  ...
  const liveInputId =
    process.env.AGV_CLOUDFLARE_LIVE_INPUT_ID ||
    process.env.CLOUDFLARE_LIVE_INPUT_ID ||
    process.env.AGV_CLOUDFLARE_INPUT_ID ||
    process.env.CLOUDFLARE_INPUT_ID ||
    "";

  return {
    accountId: String(accountId || "").trim(),
    apiToken: String(apiToken || "").trim(),
    liveInputId: String(liveInputId || "").trim(),
    accountIdConfigured: Boolean(String(accountId || "").trim()),
    apiTokenConfigured: Boolean(String(apiToken || "").trim()),
    liveInputIdConfigured: Boolean(String(liveInputId || "").trim()),
  };
}

function agvCloudflareRecordingReady() {
  const config = agvCloudflareRecordingConfig();
  return Boolean(config.accountId && config.apiToken && config.liveInputId);
}

function agvCloudflareLiveInputUrl() {
  const config = agvCloudflareRecordingConfig();
  if (!config.accountId || !config.liveInputId) return "";
  return `https://api.cloudflare.com/client/v4/accounts/${encodeURIComponent(config.accountId)}/stream/live_inputs/${encodeURIComponent(config.liveInputId)}`;
}

function agvSafeCloudflareErrors(data) {
  if (!data) return [];

  if (Array.isArray(data.errors)) {
    return data.errors.map((item) => {
      if (!item) return "Unknown Cloudflare error";
...
function agvCloudflareRecordingModeFromResult(data) {
...
async function agvFetchCloudflareLiveInput() {
  const config = agvCloudflareRecordingConfig();

  if (!agvCloudflareRecordingReady()) {
    return {
      ok: false,
      status: 500,
      data: null,
      error: "Cloudflare recording control is not configured. Check AGV_CLOUDFLARE_ACCOUNT_ID, AGV_CLOUDFLARE_API_TOKEN, and AGV_CLOUDFLARE_LIVE_INPUT_ID.",
    };
  }

  const response = await fetch(agvCloudflareLiveInputUrl(), {
    method: "GET",
    headers: {
  ...
  if (!response.ok || data?.success === false) {
    const errors = agvSafeCloudflareErrors(data);
    return {
      ok: false,
      status: response.status,
      data,
      error: errors[0] || `Cloudflare live input lookup failed with status ${response.status}.`,
    };
  }
...
async function agvSetCloudflareRecordingMode(mode) {
  const allowed = ["off", "automatic"];
  ...
  if (!allowed.includes(mode)) {
    return {
      ...
      status: 400,
      data: null,
      error: "Invalid recording mode. Use off or automatic.",
    };
  }
  ...
  if (!response.ok || data?.success === false) {
    ...
    return {
      ...
      status: response.status,
      data,
      error: errors[0] || `Cloudflare recording update failed with status ${response.status}.`,
    };
  }
...
app.get("/api/broadcast/cloudflare/recording/status", async (req, res) => {
  try {
    const config = agvCloudflareRecordingConfig();
    const result = await agvFetchCloudflareLiveInput();

    if (!result.ok) {
      return res.status(result.status || 500).json({
        ok: false,
        service: "AGV Cloudflare Recording Mode Control",
        pass: "BCAST-6",
        configured: {
          accountIdConfigured: config.accountIdConfigured,
          apiTokenConfigured: config.apiTokenConfigured,
          liveInputIdConfigured: config.liveInputIdConfigured,
        },
        error: result.error,
        timestamp: new Date().toISOString(),
      });
    ...
    const mode = agvCloudflareRecordingModeFromResult(result.data);

    return res.json({
      ok: true,
      service: "AGV Cloudflare Recording Mode Control",
      pass: "BCAST-6",
      recordingMode: mode || "unknown",
      recordingOff: mode === "off",
      recordingAutomatic: mode === "automatic",
      configured: {
        accountIdConfigured: config.accountIdConfigured,
        apiTokenConfigured: config.apiTokenConfigured,
        liveInputIdConfigured: config.liveInputIdConfigured,
...
app.post("/api/broadcast/cloudflare/recording/off", async (req, res) => {
  try {
    const result = await agvSetCloudflareRecordingMode("off");

    if (!result.ok) {
      return res.status(result.status || 500).json({
        ok: false,
        service: "AGV Cloudflare Recording Off",
        pass: "BCAST-6",
        error: result.error,
        timestamp: new Date().toISOString(),
      });
    ...
    return res.json({
      ...
      service: "AGV Cloudflare Recording Off",
      pass: "BCAST-6",
      recordingMode: mode || "off",
      message: "Cloudflare live input recording mode is off.",
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
    return res.status(500).json({
...
app.post("/api/broadcast/cloudflare/recording/automatic", async (req, res) => {
  try {
    const result = await agvSetCloudflareRecordingMode("automatic");

    if (!result.ok) {
      return res.status(result.status || 500).json({
        ok: false,
        service: "AGV Cloudflare Recording Automatic",
        pass: "BCAST-6",
        error: result.error,
        timestamp: new Date().toISOString(),
      });
    ...
    return res.json({
      ...
      service: "AGV Cloudflare Recording Automatic",
      pass: "BCAST-6",
      recordingMode: mode || "automatic",
      message: "Cloudflare live input recording mode is automatic.",
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
    return res.status(500).json({
